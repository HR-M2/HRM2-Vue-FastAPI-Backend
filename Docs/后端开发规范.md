...
└── services/agents/  # AI Agent 业务逻辑层
```
...
- Prompt 定义为模块级常量，使用 `""".strip()` 去除首尾空白
- 使用 `{placeholder}` 占位符，通过 `.format()` 填充
- 每个 Agent 角色有独立的 Prompt 常量

### 6.3 继承 BaseAgentManager
...
class MyAgentManager(BaseAgentManager):
    ...
    def setup(self):
        ...
        # 4. 创建管理器
        self.create_manager(system_message=MANAGER_PROMPT)
