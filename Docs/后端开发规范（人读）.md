...
**强制要求**：所有 Agent 的 Prompt **必须**从 YAML 文件加载，**禁止**在 Python 代码中硬编码 Prompt 字符串。

**YAML 文件规范**：

```yaml
# prompts/{agent_name}.yaml
...
**变量替换**：使用 `{variable}` 占位符，调用时通过 `get_prompt()` 替换

```python
# ✅ 正确：从 YAML 加载
system_message = get_prompt("screening", "hr_system", hr_rules=rules_json)

# ❌ 禁止：硬编码 Prompt
system_message = f"""你是 HR 专家，请按以下规则评分：{rules_json}"""
```

**新增 Prompt 流程**：
1. 在 `app/agents/prompts/{agent}.yaml` 中添加新键
2. 在 Agent 代码中通过 `get_prompt("agent", "key", **vars)` 调用
3. 开发环境支持热加载，修改 YAML 后无需重启
